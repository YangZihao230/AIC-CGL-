...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        ### Multi level ViT for vit_base_patch16_224_miil_in21k (depth=12, num_heads=12)
        stage1_depth = [0, 1]
        stage2_depth = [2, 3]
        stage3_depth = [4, 5, 6, 7, 8, 9]
        stage4_depth = [10, 11]
        ## Origin
        ...
        #         dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
        #         attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer)
        #     for i in stage1_depth])
        # self.stage2_blocks = nn.Sequential(*[
        #     Block(
        #         dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
        #         attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer)
        #     for i in stage2_depth])
        # self.stage3_blocks = nn.Sequential(*[
        #     Block(
        #         dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
        #         attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer)
        #     for i in stage3_depth])
        # self.stage4_blocks = nn.Sequential(*[
        #     Block(
        #         dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
        #         attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer)
        #     for i in stage4_depth])
        ### Multi level ViT for vit_base_patch16_224_miil_in21k
    ...
    def forward_features(self, x):
        ...
        x4 = x.clone()
        feature_dict = {}
        feature_dict['layer1'] = x1
        feature_dict['layer2'] = x2
        feature_dict['layer3'] = x3
        feature_dict['layer4'] = x4
        return feature_dict
