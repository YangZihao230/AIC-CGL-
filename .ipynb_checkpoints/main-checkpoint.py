...
def train(args, epoch, model, scaler, amp_context, optimizer, schedule, train_loader):
    ...
    # = = = 新增：初始化训练指标累积变量（与验证逻辑对齐）= = =
    train_total = 0  # 训练总样本数（同验证集统计逻辑）
    train_correct = 0  # 训练正确样本数（同验证acc的分子）
    train_running_loss = 0.0  # 训练累积损失（与验证损失单位一致）
    ...
    for batch_id, (ids, datas, labels) in enumerate(train_loader):
        ...
        """ = = = = 新增：累积训练指标（与验证逻辑对齐）= = = = """
        # 损失：还原为实际值（抵消update_freq的除法，与验证损失单位一致）
        current_loss = loss.item() * args.update_freq
        train_running_loss += current_loss

        # 准确率：与验证acc计算逻辑完全一致（正确数/总样本数×100）
        # 优先用验证逻辑可能采用的输出（comb_outs→ori_out→layer4），确保指标可比
        if "comb_outs" in outs:
            preds = torch.argmax(outs["comb_outs"], dim=1)
        elif "ori_out" in outs:
            preds = torch.argmax(outs["ori_out"], dim=1)
        else:
            preds = torch.argmax(outs["layer4"].mean(1), dim=1)
        
        train_total += batch_size
        train_correct += (preds == labels).sum().item()
        train_acc = train_correct / train_total * 100  # 转为百分比（同验证acc格式）
        ...
        #     msg['info/epoch'] = epoch + 1
        #     msg['info/lr'] = get_lr(optimizer)
        #     # 新增：添加训练损失和准确率（与验证accs键名对应，便于Wandb对比）
        #     msg['train/avg_loss'] = train_running_loss / args.log_freq
        #     msg['train/acc'] = train_acc
        #     cal_train_metrics(args, msg, outs, labels, batch_size)
        #     wandb.log(msg)

        """ = = = = 新增：实时打印训练指标（与验证日志格式对齐）= = = = """
        if (batch_id + 1) % args.log_freq == 0:
            avg_loss = train_running_loss /args.log_freq
            # 格式模仿验证的"....BEST_ACC"，前缀统一用"...."，保持输出风格一致
            print(f"\n....Train Epoch {epoch+1} Batch {batch_id+1}/{total_batchs}")
            print(f"....Train Loss: {avg_loss:.4f} | Train Acc: {train_acc:.2f}%")
            # 重置累积损失（准确率累积到epoch结束，不重置）
    ...
    """ = = = = 新增：每个Epoch结束打印整体训练指标（与验证Epoch级结果对应）= = = = """
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if epoch == 0 or (epoch + 1) % args.eval_freq == 0:
            ...
            if acc > best_acc:
                ...
                torch.save(checkpoint, args.save_dir + "backup/best_.pt")
